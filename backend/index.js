...
const CATEGORY_KEYWORDS = {
  TOURS: ["tour", "guided", "walk", "walking", "excursion", "sightseeing", "guide"],
  RESTAURANT: ["restaurant", "food", "dining", "wine", "tasting", "culinary", "bistro"],
  MUSEUM: ["museum", "gallery", "exhibit", "exhibition", "art", "heritage"],
  NIGHTLIFE: ["nightlife", "club", "bar", "pub", "cabaret", "burlesque", "show"],
  SHOPPING: ["shopping", "market", "boutique", "mall", "fashion", "souvenir"],
  SIGHTS: ["tower", "landmark", "monument", "cathedral", "basilica", "palace", "castle", "bridge", "view", "panorama", "lookout"],
};
...
async function resolveCityGeo({ city, countryCode }, { amadeus, googleApiKey }) {
  ...
  try {
    ...
    if (withGeo) return { name: withGeo.name || withGeo.iataCode || city, geoCode: withGeo.geoCode };
  } catch (e) {
    // swallow and try next strategy
  }
  ...
  if (googleApiKey) {
    try {
      const url = `https://maps.googleapis.com/maps/api/geocode/json?address=${encodeURIComponent(
        city
      )}${countryCode ? `&components=country:${encodeURIComponent(countryCode)}` : ''}&key=${googleApiKey}`;
      ...
      if (first?.geometry?.location) {
        return {
          ...
          geoCode: { latitude: first.geometry.location.lat, longitude: first.geometry.location.lng },
        };
      }
    } catch (e) {
      // ignore
...
app.get("/api/activities/by-city", async (req, res) => {
  try {
    ...
    if (minPrice != null && !Number.isNaN(minPrice)) {
      list = list.filter((a) => !Number.isNaN(priceNum(a.price)) && priceNum(a.price) >= minPrice);
    }
    if (maxPrice != null && !Number.isNaN(maxPrice)) {
      list = list.filter((a) => !Number.isNaN(priceNum(a.price)) && priceNum(a.price) <= maxPrice);
    }
    if (category) {
    ...
    return res.status(500).json({ error: "Activities by-city failed", details });
  }
});
